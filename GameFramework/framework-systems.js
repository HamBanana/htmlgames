// GameFramework/framework-systems.js - Game systems (clean, no redeclarations)

/**
 * Input System - Handles keyboard and mouse input
 */
if (typeof window !== 'undefined' && !window.InputSystem) {
 ...
 */
if (typeof window !== 'undefined' && !window.PhysicsSystem) {
    class PhysicsSystem extends System {
        ...
        update(deltaTime) {
            const entities = this.game.getEntitiesWithComponent(window.PhysicsComponent);
            
            entities.forEach(entity => {
                const physics = entity.getComponent(window.PhysicsComponent);
 ...
 */
if (typeof window !== 'undefined' && !window.AsepriteParser) {
 ...
 */
if (typeof window !== 'undefined' && !window.RenderSystem) {
    class RenderSystem extends System {
        ...
        async loadAseprite(name, filename) {
            try {
                ...
                const spriteData = window.AsepriteParser.parse(jsonData);
 ...
 */
if (typeof window !== 'undefined' && !window.CameraSystem) {
...
if (typeof window !== 'undefined' && !window.CollisionSystem) {
    class CollisionSystem extends System {
        ...
        update(deltaTime) {
            const collisionEntities = this.game.getEntitiesWithComponent(window.CollisionComponent);
            ...
            for (let i = 0; i < collisionEntities.length; i++) {
                for (let j = i + 1; j < collisionEntities.length; j++) {
                    ...
                    const collisionA = entityA.getComponent(window.CollisionComponent);
                    const collisionB = entityB.getComponent(window.CollisionComponent);
